    ...
    def _check_connection(self, host_list):
        ...
        if unreachable_hosts:
            for host in unreachable_hosts:
                print_error(f"Unable to connect to memcache server: {host}")
            sys.exit(1)
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print_warning(f'No hosts specified, defaulting to {host_list[0]}')
    # Some banner fun to make our CLI all warm and fuzzy
    print(f"Connecting to {', '.join(host_list)}")
